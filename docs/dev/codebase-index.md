...
| Inventory stock movement | `modules/inventory/stock_movement.py`, `modules/inventory/module.py` | W5 `STOCK_MOVEMENT` / `STOCK_MOVEMENT_LINE`; D1–D4 locked in `plan/20` §1.5 |
| Entity validators hook | `platform/api/src/emcap/module/loader.py` `load_entity_validators`, `api/routes/entities.py` | Dispatches module `ENTITY_VALIDATORS` on create/update |
...
| Web metadata fixtures | `clients/web/src/assets/fixtures/metadata/` | Mirror of `platform/api/tests/fixtures/metadata/` for Karma (PRODUCT, WAREHOUSE, LEAD, CONTACT) |
...
| Web settings (Phase 12) | `clients/web/src/app/pages/settings/` | Module toggles, templates, P17-T11 rule evaluate (`/app/settings/rules`) |
...
| Mobile workflow inbox | `clients/mobile/lib/app/workflow_inbox_screen.dart` | P17-T02 filters, SLA, entity nav, detail panel |
...
| `shared/utils/` | export, page-title, record, tenant, **document-preview**, **assistant-chat** helpers |
...
| `platform/api/tests/test_entity_system_contract.py` | P20-T05 parametrized S1–S3 for all registry entities + W1 fixture key snapshots |
| `platform/api/tests/test_crm_entity_fields.py` | P20-T09 LEAD enum status, CONTACT lead lookup, status chip metadata |
| `platform/api/tests/test_stock_movement_entities.py` | P20-T17 W5 movement enum, line lookup chain, transfer source warehouse rule |
...
| `clients/web/src/app/metadata/entity-system.fixture.spec.ts` | P20-T10 W1 entity form/grid fixture system-section + headline contract |
...
| `clients/mobile/test/support/entity_fixtures.dart` | P20-T11 generic loader for `{entity}.*.json` API metadata fixtures |
| `clients/mobile/test/entity_system_contract_test.dart` | P20-T11 W1 parametrized system-section + grid keys (fixture-backed) |
| `clients/mobile/test/record_headline_test.dart` | P20-T11 metadata-driven record headline (W1 entities) |
...
| `clients/mobile/lib/app/workflow_inbox_screen.dart` | P17-T02 workflow inbox (filters, SLA, entity nav) |
