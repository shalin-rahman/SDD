...
| | |
|--|--|
| **Symptom** | `npm ci` fails: `peer @angular/core@"19.2.0" from @angular/animations@19.2.0` while lockfile has `@angular/core@19.2.25` |
| **Cause** | `^19.2.0` resolves framework packages to latest 19.2.x; older lock entries (animations/cdk/material at 19.2.0) keep strict exact peers; Material/CDK publish fewer 19.2.x patches than `@angular/core` |
| **Fix** | Pin framework `@angular/*` to the same patch (e.g. `19.2.25`) without carets; pin `@angular/cdk` and `@angular/material` to the latest available 19.2.x (e.g. `19.2.19`); run `npm install` in `clients/web` and commit `package-lock.json`. Do **not** rely on `--legacy-peer-deps` (CI uses plain `npm ci`). |
| **Test** | `cd clients/web && npm ci` (Linux CI) or `npm install --dry-run` |
