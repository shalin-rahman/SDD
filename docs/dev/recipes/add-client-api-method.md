...
1. **Web** — `clients/web/src/api/emcap-client.ts` — add typed method using `this.request()`
2. **Web contract test** — `clients/web/src/api/emcap-client.test.ts` — add to `REQUIRED_METHODS`
3. **Mobile** — `clients/mobile/lib/api/emcap_client.dart` — add `_request()` wrapper method
4. **Shell UI** (optional) — `clients/web/src/app/main.ts` or `clients/mobile/lib/app/`
